...
import { format, addDays, startOfWeek, parseISO, isSameDay, addMinutes } from 'date-fns';
...
  const getAppointmentsForSlot = (day: Date, slotStart: Date): PopulatedAppointment[] => {
...
            <th className="w-24 p-2 text-xs font-semibold text-gray-700 sticky left-0 bg-gray-50">Time</th>
            {weekDays.map((day, index) => (
              <th key={index} className="p-2 border-l border-gray-200 text-xs font-semibold text-gray-700 w-40 sm:w-auto">
                <div className="font-bold">{format(day, 'EEE')}</div>
                <div className="text-gray-600 font-normal">{format(day, 'MMM d')}</div>
              </th>
            ))}
          </tr>
        </thead>
...
            <tr key={slotIndex}>
              <td className="p-1 text-xs text-gray-500 w-24 align-top border-r border-gray-200 sticky left-0 bg-white" style={{ height: '48px' }}>
                {slot.label}
              </td>
...
                
                return (
                  <td key={dayIndex} className="p-1 border-l border-gray-100 align-top" style={{ height: '48px' }}>
                    {slotAppointments.map(apt => (
                      <div 
                        key={apt.id} 
                        className="text-[10px] rounded p-0.5 mb-0.5 text-white overflow-hidden font-medium truncate" 
                        style={{ backgroundColor: APPOINTMENT_TYPE_CONFIG[apt.type].color }}
                        title={`${apt.patient.name} - ${APPOINTMENT_TYPE_CONFIG[apt.type].label}`}
                      >
                        {apt.patient.name}
                      </div>
                    ))}
